  ...
  public class Writing {
	...
	private static void writeWithCsvBeanWriter() throws Exception {
		...
		final CustomerBean john = new CustomerBean("1", "John", "Dunbar",
  			...
  			null, "1600 Amphitheatre Parkway\nMountain View, CA 94043\nUnited States", null, null,
...
final Object[][] johnData = new Object[][] {{"1", "John", "Dunbar",
	...
	final ResultSet john = new ResultSetMock(johnData, header);
 		...
 		try {
 			resultSetWriter = new CsvResultSetWriter(new FileWriter("/writeWithCsvResultSetWriter.csv"),
		CsvPreference.STANDARD_PREFERENCE);
 			final CellProcessor[] processors = getProcessors();
...
// create the customer beans
 		...
 		ICsvBeanWriter beanWriter = null;
 		try {
 			beanWriter = new CsvBeanWriter(new FileWriter("/partialWriteWithCsvBeanWriter.csv"),
 				CsvPreference.STANDARD_PREFERENCE);
 	...
 	private static void partialWriteWithCsvListWriter() throws Exception {
 		...
 		ICsvListWriter listWriter = null;
 		try {
 			listWriter = new CsvListWriter(new FileWriter("/partialWriteWithCsvListWriter.csv"),
 				CsvPreference.STANDARD_PREFERENCE);
...
*/
	...
	john.put(header[3], null);
 		...
 		ICsvMapWriter mapWriter = null;
 		try {
		mapWriter = new CsvMapWriter(new FileWriter("/partialWriteWithCsvMapWriter.csv"),
 				CsvPreference.STANDARD_PREFERENCE);
