...
public class EntityCsvMapper {
    ...
    public  void writeCsdWorkcenters(PrintWriter writer, List<CsdWorkcenter> wcs)  {
        ...
        try {
            ...
            String[] columns = new String[]{"bomItemLevel","qty", "partNumber", "partTitle", "edrnNieoRev", 
            		"installedBy","toDwg", "dirReleaseDate", "dataSetType", "cageCode", "modSumNo", "materialGrp", "cahierName"};
          
            
            mapStrategy.setColumnMapping(columns);
            
            StatefulBeanToCsv btcsv = new StatefulBeanToCsvBuilder(writer).withQuotechar(CSVWriter.DEFAULT_ESCAPE_CHARACTER)
	...
	public List<CsdWorkcenter> toCsdWorkcenters(com.bombardier.macs.model.Workcenter csdWc, List<String> cahierIds,  List<MasterPart> parts, List<Part> piParts) {
	...
	private MasterPart  getParentMasterPart(String id, List<MasterPart> parts){
	 ...
	 public int calculateLevel(Part part,  int level, List<Part> parts){
		    if(part==null)return level;
			if(part.getParentId()==null || part.getId()==part.getParentId() ) return level;
			else {
